...
def svg_name_for(glyph_name: str) -> str:
    """Resolve glyph name to source SVG stem (e.g. claude_small -> claude)."""
    return glyph_name[:-6] if glyph_name.endswith("_small") else glyph_name


def fit_for(fits: dict, name: str) -> float:
    """Per-glyph scale, falling back to small_scale * base or `default` then to 1.0."""
    if name in fits:
        return float(fits[name])
    if name.endswith("_small"):
        base_name = name[:-6]
        return fit_for(fits, base_name) * float(fits.get("small_scale", 0.72))
    return float(fits.get("default", 1.0))
...
def build(output: Path) -> None:
    ...
    for name in names:
        svg_stem = svg_name_for(name)
        svg_path = ROOT / "tools" / "svg" / f"{svg_stem}.svg"
...
def selfcheck(path: Path, fits: dict | None = None) -> None:
    ...
    for name in font.getGlyphOrder():
        ...
        bounds_pen = BoundsPen(None)
        svg_stem = svg_name_for(name)
        SVGPath(str(ROOT / "tools" / "svg" / f"{svg_stem}.svg")).draw(bounds_pen)
