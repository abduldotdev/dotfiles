...
- **Built-in Herdr theme accent resolution.**
  ...
  A table of built-in accents transcribed from Herdr v0.9.1 maps themes like
  `tokyo-night-day` to their native hex accent (`#2e7de9`), resolving aliases
  and respecting `auto_switch` with light/dark appearance. Resting workspace
  names, ahead counts, and Agents panel headers now draw in the active Herdr
  theme's accent, while hand-written `[theme.custom]` overrides continue to
  take precedence and `terminal` still falls through to omarchy.

- **Theme-accented Agents panel workspace headers.**
  Workspace headers (`$group` and `$group_parent`) in the Agents panel are now
  drawn in the live theme accent resolved from `[theme.custom]` in `config.toml`
  or omarchy's `colors.toml`, keeping them visually consistent with resting
  workspace names in the Spaces panel. If no accent source is resolved, they fall
  back to their respective palette inks (`state.subtle` and `state.idleStale`).
  Fully stale groups (`$group_stale`) remain in dim palette grey so inactive
  workspaces still recede.

- **Spaces branch row shares the worktree mark.**
  The Spaces checkout row (`$space_branch`) now uses `config.worktreeMark`
  (default Octicons `U+F418`, configurable via `worktree_mark`) instead of
  hardcoding Powerline's branch mark. The Spaces panel and Agents panel now
  share the same git-branch glyph and stay consistent, including publishing
  the bare branch name with no leading space when `worktree_mark` is set
  to `""`.

- **Theme-accented ahead counts and resting workspace names.**
  Ahead counts (`$space_ahead`) are now drawn in the live theme accent, resolved
  as a hex value from `[theme.custom]` in `config.toml` or omarchy's current
  theme `colors.toml` (falling back to palette green when absent). Herdr's
  `terminal` theme paints its accent from the terminal palette, but sidebar cells
  accept only hex colours, so `lib/accent.js` detects the active accent and
  refreshes the managed sidebar block when it changes. Resting workspace state
  cells (`$space_idle`, `$space_unknown`, `$space_none`) are now drawn in the
  theme accent as well (falling back to palette grey when absent), while active
  states remain in their recognisable signal colours.

- **Decluttered Spaces rows and ahead-of-upstream commit counts.**
  The Spaces column now presents cleaner, more compact workspace rows with
  two changes:

  - **Icon-only vendor logos.** The Spaces logo row now publishes glyphs alone
    without spelling out the vendor name (e.g. `󰛄` instead of `󰛄 claude`). The
    Spaces column is narrow and the Agents panel already spells the vendor name
    beside each running session, so repeating the word crowded the line.
    Unmarked vendors without a glyph still fall back to their name so an
    unrecognised harness is not invisible.
  - **Ahead-of-upstream commit counts.** A new `$space_ahead` workspace token
    displays the number of commits a workspace checkout is ahead of its
    upstream tracking branch (rendered as `↑N`, e.g. `↑3`), positioned directly
    after the workspace name (`<mark> <name> · ↑3`). It is rendered in
    `state.done` green to signal pending work ready to integrate without alarm.
    Computation is non-blocking, caching ahead counts per checkout path and
    spawning `git rev-list` only when filesystem mtimes change or a 30-second
    rate-limiting floor elapses.

- **Smaller vendor mark variants for Spaces.** Added small glyph variants
  (`_small`) for all 24 vendor marks at `U+E1D0`–`U+E1E7` in the bundled icon
  font, scaled to ~0.72× of each vendor's fit via `small_scale = 0.72` in
  `tools/codepoints.toml` so marks remain crisp and legible in 12px cells.
  `lib/logos.js` now exports `PUA_SMALL` and `smallLogoFor()`, and `lib/font.js`
  derives the third codepoint range for terminal config mappings.
