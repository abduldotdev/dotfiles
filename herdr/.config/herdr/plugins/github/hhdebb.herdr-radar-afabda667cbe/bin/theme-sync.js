...
function sync({ force = false } = {}) {
  ...
  if (variant) {
    ...
    if (result.ok && result.changed) appearanceChanged = true;
  }
  const accentResult = managed.refreshSidebarAccent();
  const accentChanged = Boolean(accentResult?.changed);
  if (appearanceChanged || accentChanged) reloadConfig();
  const currentAccent = accent.current() ?? 'none';
  return {
    changed: appearanceChanged || accentChanged,
    appearanceChanged,
    accentChanged,
    variant: variant ?? appearance.current(),
...
if (require.main === module) {
  ...
  if (mode === '--check') {
    ...
    console.log(`accent: ${accent.current() ?? 'none'}`);
  } else {
    const result = sync({ force: mode === '--force' });
    if (result.off) {
      console.log('follow_appearance = false; nothing to do (use --force to apply once)');
    } else {
      const parts = [];
      if (result.appearanceChanged) parts.push(`theme block -> ${result.variant}`);
      if (result.accentChanged) parts.push(`accent -> ${result.accent}`);
      console.log(parts.length ? parts.join(', ') : `unchanged (${result.variant ?? 'unknown'})`);
    }
  }
}
