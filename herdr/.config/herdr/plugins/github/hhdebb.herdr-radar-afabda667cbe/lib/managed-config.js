...
// A cell with no colour of its own: it takes whatever Herdr's theme gives the
// row, and follows the theme when that changes. The palette's greys are tuned
// against the plugin's own theme block, which is fine for a state and wrong
// for plain text — the branch row is prose, and prose should look like the
// rest of the panel.
function themedCell(token, bold = false, dim = false) {
  return `{ token = "${token}", bold = ${bold}, dim = ${dim} }`;
}
...
const ACCENT_TAG = '# accent: ';

function blockAccent(text) {
  return new RegExp(`^${escapeRegExp(ACCENT_TAG)}(#[0-9a-f]{6}|none)$`, 'm').exec(text ?? '')?.[1] ?? null;
}
...
function sidebarBlock(variant) {
  ...
  const agentRow = (vendor) =>
    [
      ...
      // entry, where the row collapses. Drawn in the live theme accent to
      // match the rest of the workspace headers, falling back to the palette's
      // stale tier when no accent source exists. The terminal's `dim` used to
      // be asked for as well, and on a dark panel that took the row out of reach.
      `[${[cell('$group_parent', accent.current() ?? state.idleStale, true)].join(', ')}]`,
      // Workspace headers follow the live theme accent, consistent with the
      // resting Space names, falling back to the palette's subtle ink when no
      // accent is resolved. The header fades with its workspace: when every
      // session inside has gone stale the name shifts to `$group_stale` and
      // dims to the palette's stale tier — leaving it at full strength put a
      // column of bright names over faded contents, which reads as damage
      // rather than as a project that is simply asleep.
      `[${[cell('$group', accent.current() ?? state.subtle, true), cell('$group_stale', state.idleStale, true)].join(', ')}]`,
      `[${[
  ...
  // Row one is `<mark> <name> · <marks>`: the state mark and the workspace
  // name in ONE cell (lib/state.js writeSpaceState says why), then the
  // vendors running there. Row two is the checkout, `<branch> · ↑N`, the
  // ahead count beside the branch where a git prompt would put it.
  //
  // Every state cell is bold, not just the loud ones: the name rides in the
  // cell, the name is the anchor of the entry, and a name that flipped weight
  // with the state would read as a fault. Blocked/working/done keep their
  // signal colours, while resting states ($space_idle, $space_unknown,
  // $space_none) wear the theme ACCENT — the same colour Herdr gives the active
  // tab — resolved by lib/accent and refreshed with it, falling back to the
  // palette's `none` grey when no accent source exists. The vendor marks used
  // to have the second row to themselves, one cell per vendor; they belong
  // beside the name, and that row is at Herdr's sixteen-token ceiling with
  // fourteen state marks on it, so they share one cell there
  // (lib/state.js SPACE_TOKENS). A named working mark per branded vendor;
  // everything else shares `other`. The roster used to be kept short because a
  // workspace wrote its whole token set in one patch and Herdr rejects a patch
  // over sixteen tokens whole — that is chunked now (lib/state.js), so the list
  // can simply follow the palette.
  const spaceMarks = [
    ...
    cell('$space_idle', accent.current() ?? state.none, true),
    cell('$space_unknown', accent.current() ?? state.none, true),
    cell('$space_none', accent.current() ?? state.none, true),
    // Every vendor in the Space, packed. The rules are the SMALL glyph table:
    // a rule reads the cell's value and the daemon publishes small marks
    // here, so the full-size table would match nothing. First match wins, and
    // the daemon puts branded vendors first, so a single-vendor Space wears
    // its brand and a mixed one wears the first's. Same ink rule as the Agents
    // logo cell: a vendor with no hue is drawn in ink, not grey.
    logoCell('$space_logos', logos.smallGlyphs(), ink),
  ];
  // The checkout row takes the theme's own text colour rather than a palette
  // grey: it is prose beside the panel's other prose, and it should change
  // with the theme the way the rest of the panel does.
  //
  // The ahead count is drawn in the theme accent (or palette green as fallback
  // when no accent source is resolved). Herdr's `terminal` theme paints its
  // accent from the host terminal's ANSI palette, but sidebar cells accept
  // only a hex `#rrggbb` — so lib/accent resolves the active accent (from a
  // custom override or omarchy's current theme) and writes it here.
  const spaceCheckout = [themedCell('$space_branch'), cell('$space_ahead', accent.current() ?? state.done, true)];

  // Herdr's 16-token ceiling applies to every sidebar row, Spaces included.
  for (const row of [`[${spaceMarks.join(', ')}]`, `[${spaceCheckout.join(', ')}]`]) {
    const count = tokensIn(row);
    if (count > ROW_TOKEN_LIMIT) {
      throw new Error(`sidebar space row would carry ${count} tokens; Herdr allows ${ROW_TOKEN_LIMIT}`);
    }
  }
  ...
  return [
    ...
    `${ACCENT_TAG}${accent.current() ?? 'none'}`,
    ...
    // of them is a lot of air beside a two-row entry — the branch row already
...
function refreshSidebarAccent() {
  const report = inspect();
  if (!report.text || !report.text.includes(SIDEBAR_START)) {
    return { ok: true, changed: false };
  }
  const currentAccent = accent.current() ?? 'none';
  if (blockAccent(report.text) === currentAccent) {
    return { ok: true, changed: false };
  }
  backup(report.file);
  const pattern = new RegExp(`${escapeRegExp(SIDEBAR_START)}[\\s\\S]*?${escapeRegExp(SIDEBAR_END)}`);
  const next = report.text.replace(pattern, sidebarBlock(chromeVariant(report.text) ?? 'light'));
  writeAtomic(report.file, next, identity.TMP_SUFFIX);
