...
async function start() {
  ...
  if (config.followAppearance) {
    ...
    // Accent follow-up. Herdr's `terminal` theme paints the accent from the
    // host terminal's ANSI palette, which changes when the theme flips in
    // omarchy or is edited in config.toml. Accent follows the same
    // `followAppearance` switch. Spawning theme-sync.js is deliberate (same as
    // appearance above) so code on disk rewrites the block.
    let lastAccentSig = accent.signature();
    timers.push(
      setInterval(() => {
        try {
          const nextSig = accent.signature();
          if (nextSig !== lastAccentSig) {
            lastAccentSig = nextSig;
            detachedNode(path.join(__dirname, '..', 'bin', 'theme-sync.js'));
          }
        } catch (error) {
          logError(error);
