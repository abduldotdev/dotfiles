...
async function labels(now) {
  ...
  for (const ws of list) {
    if (typeof ws.workspace_id === 'string') {
      ...
      if (typeof ws.worktree?.checkout_path === 'string') {
        checkouts.set(ws.workspace_id, ws.worktree.checkout_path);
      }
    }
  }
  // Herdr attaches a `worktree` object only to the workspaces it has scanned
  // as a repository; in practice close to half of an ordinary session's
  // workspaces come without one — the ones opened straight into a directory,
  // or before the scan ran. The ahead count needs *some* path to ask git about,
  // and the workspace's first pane is sitting in one. lib/git.js walks up from
  // wherever that is to find the `.git`, so a shell that has cd'd into a
  // subdirectory still resolves to the right checkout.
  if (checkouts.size < workspaces.size) {
    for (const pane of await herdr.panesAsync()) {
      const ws = pane.workspace_id;
      if (typeof ws !== 'string' || checkouts.has(ws)) continue;
      const cwd = typeof pane.cwd === 'string' && pane.cwd ? pane.cwd : pane.foreground_cwd;
      if (typeof cwd === 'string' && cwd) checkouts.set(ws, cwd);
    }
  }
  const { parents, worktrees } = worktreeParents(list);
  if (tabs.size > 0) Object.assign(cache, { at: now, tabs, workspaces, parents, worktrees, checkouts });
  return cache;
}
...
const SPACE_TOKENS = [
  ...
  'space_unknown',
  'space_none',
  // Not a state: the vendors alive in the workspace, as small marks packed
  // into ONE cell on the name row. They used to be one cell per vendor on a
  // row of their own, each in its brand colour — but the name row is where
  // they belong, and that row is already at Herdr's sixteen-token ceiling
  // once the fourteen state marks and the name are on it. One cell is what
  // fits. A value rule in the sidebar block still colours the cell by the
  // first vendor mark it contains, so a single-vendor Space keeps its brand
  // and a mixed one wears the first's.
  'space_logos',
  // The branch the checkout is on, for the second row.
  'space_branch',
  // Commits pending push to upstream (e.g. "↑3"), or null when at or behind
  // upstream. Sits beside the branch, where a git prompt would put it.
  'space_ahead',
];

// Cells this plugin published under an earlier layout and no longer draws.
// A token Herdr was never told to forget stays on the workspace, and an old
// block would still render it — so every Spaces write nulls these too. Drop
// them a release after nobody can have them left. `space_label` was the name
// on a cell of its own; it rides inside the state mark's cell now (see
// writeSpaceState).
const LEGACY_SPACE_TOKENS = [
...
// The vendors alive in a workspace, as logo glyphs only (no vendor names),
// packed into the one `space_logos` cell. The Spaces column is narrow and the
// Agents panel already spells out the vendor beside each session, so the name
...
function spaceLogoTokens(agents) {
  ...
  for (const a of agents) {
    ...
    seen.add(a.name);
    marks.push({ vendor: a.name, glyph: smallLogoFor(a.name) ?? a.name });
  }
  const rank = (vendor) => {
    const index = palette.brandVendors.indexOf(vendor);
    return index === -1 ? palette.brandVendors.length : index;
  };
  marks.sort((a, b) => rank(a.vendor) - rank(b.vendor));
  return { space_logos: marks.length ? marks.map((m) => m.glyph).join(' ') : null };
}
...
function writeSpaceState(
  ...
  const tokens = {};
  // The mark and the name share ONE cell, `<mark> <name>`, the way an Agent
  // row does: Herdr joins adjacent cells with ` · `, and a dot between a mark
  // and the name it marks read as punctuation in the wrong place. The price
  // is that one cell is one colour, so the name wears the state's colour —
  // the vendor's while something works there, grey once it is parked — which
  // is the Agents panel's language already. (Herdr's built-in `workspace`
  // cell draws the real label and cannot be styled or merged, which is why
  // the name is published by the plugin at all.)
  const value = label ? `${glyph} ${label}` : glyph;
  for (const name of [...SPACE_TOKENS, ...LEGACY_SPACE_TOKENS]) tokens[name] = name === tokenName ? value : null;
  Object.assign(tokens, logoTokens);
  // The glyph in front of the branch. The Spaces row shares the worktree mark
  // with the Agents panel (config.worktreeMark, default U+F418) rather than
  // carrying its own glyph, so the two panels stay consistent. Like the
  // Agents path, setting `worktree_mark` to "" drops the mark and publishes
  // the bare branch name with no leading space.
  const mark = config.worktreeMark ? `${config.worktreeMark} ` : '';
  tokens.space_branch = branch ? `${mark}${branch}` : null;
  tokens.space_ahead = ahead;
...
function clearSpaceState(source, workspaceId) {
  const tokens = {};
  for (const name of [...SPACE_TOKENS, ...LEGACY_SPACE_TOKENS]) tokens[name] = null;
  return reportChunked(herdr.reportWorkspaceMetadataAsync, workspaceId, source, tokens);
}
