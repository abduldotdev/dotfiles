...
class Frame {
  ...
  async render(now) {
    ...
    const { tabs, workspaces, parents, worktrees, checkouts } = await state.labels(now);
    ...
    this.spaceJobs(wsAgents, workspaces, checkouts ?? new Map(), now, jobs);
  ...
  spaceJobs(wsAgents, workspaces, checkouts = new Map(), now, jobs) {
    const src = this.src;
    ...
    for (const ws of wsIds) {
      ...
      const checkoutPath = checkouts.get(ws);
      // git.ahead is async and cached with filesystem mtime and time floor guards.
      // We read the last known cached value synchronously and kick the refresh in
      // the background without awaiting it here, ensuring the 150ms frame loop
      // remains completely non-blocking even if git or filesystem operations stall.
      // When a refreshed count arrives, the next tick's key will differ and publish.
      const ahead = checkoutPath ? git.cachedAhead(checkoutPath) : null;
      if (checkoutPath) git.ahead(checkoutPath).catch(() => {});
      // The branch is a read of `.git/HEAD`, nothing more — the tab-bar does
      // the same on its own timer, and it is what keeps this row honest after
      // an agent checks something out mid-session.
      const branch = checkoutPath ? git.branch(checkoutPath) : null;
      const key = `${token}:${glyph}:${Object.values(logos).join(',')}:${label}:${ahead ?? ''}:${branch ?? ''}`;
      if (this.lastSpace.get(ws) !== key && this.writable(`space:${ws}`, now)) {
        jobs.push(
          state.writeSpaceState(src, ws, token, glyph, logos, label, ahead, branch).then((ok) => {
