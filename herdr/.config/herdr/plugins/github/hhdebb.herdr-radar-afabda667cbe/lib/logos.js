...
// Smaller vendor marks for the Spaces column (E1D0–E1E7), scaled at ~0.72x
// in the font to remain crisp and legible in smaller cells.
const PUA_SMALL = {
  claude: '\ue1d0',
  codex: '\ue1d1',
  opencode: '\ue1d2',
...
// The small table, through the same door, for the cell that packs a Space's
// vendors together: its colour rule has to match the exact string the daemon
// publishes, and that string is a small mark.
function smallGlyphs(variant = config.variant) {
  const resolved = resolveVariant(variant);
  if (resolved === 'none') return {};
  return { ...(resolved === 'font' ? PUA_SMALL : TEXT) };
}
...
// The smaller mark for `agent`, used in the Spaces column, or null when this
// agent has none. When the variant is text it falls back to the ordinary TEXT
// glyph, since text marks have no size tiers.
function smallLogoFor(agent, variant = config.variant) {
  if (!agent || !(agent in PUA_SMALL) || variant === 'none') return null;
  const resolved = resolveVariant(variant);
  return resolved === 'font' ? PUA_SMALL[agent] : TEXT[agent];
}
